...
# Simulate trip_duration (target variable) with meaningful relation
dummy_data['trip_duration'] = (
    dummy_data['trip_distance'] * 4
    + dummy_data['fare_amount'] * 0.1
    + dummy_data['hour'] * 0.5
    + np.random.normal(0, 2, 1000)  # small noise
)

# ---------- CORRELATION HEATMAP ----------
st.subheader("📈 Feature Correlation with Trip Duration")
corr = dummy_data.corr()
fig, ax = plt.subplots(figsize=(10, 6))
sns.heatmap(corr[['trip_duration']].sort_values(by='trip_duration', ascending=False), annot=True, cmap="coolwarm", ax=ax)
st.pyplot(fig)
